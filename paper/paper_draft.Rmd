...
library(snpR); library(data.table); library(ggplot2); library(gridExtra); library(dplyr); library(openxlsx); library(jpeg); library(nlme)
...
## Sequencing results
We incubated female monarch butterflies from a total of `r length(unique(na.omit(old.data$fam)))` clutches of eggs produced by females collected from a continously-breeding population from southern Queensland, Australia under a decreasing photoperiod following the methods of Freedman et al. [-@Freedman2018]. The resulting `r nrow(old.data)` females were phenotyped for oocyte development as a proxy for diapause status. `r sum(sample.meta$likely_pheno[sample.meta$year == 2018] == "R", na.rm = TRUE)` butterflies were phenotyped as reproductively mature (e.g. not in diapause, and thus a non-migratory phenotype) and `r sum(sample.meta$likely_pheno[sample.meta$year == 2018] == "B", na.rm = TRUE)`as reproductively immature (e.g. in diapause, and thus a migratory phenotype). We sequenced these individuals along with a additional `r sum(sample.meta$year == 2016, na.rm = TRUE)` butterflies raised under very similar conditions by Freedman et al. [-@Freedman2018], of which `r sum(sample.meta[sample.meta$year == 2016,]$likely_pheno == "R", na.rm = TRUE)` and `r sum(sample.meta[sample.meta$year == 2016,]$likely_pheno == "M", na.rm = TRUE)` were scored in that study as reproductively mature and immature, respectively.
