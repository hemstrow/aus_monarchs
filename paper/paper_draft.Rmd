...
library(snpR); library(data.table); library(ggplot2); library(gridExtra); library(dplyr); library(openxlsx); library(imager); library(nlme)
...
# top SNP
top_snp <- dplyr::arrange(get.snpR.stats(dat, stats = c("association", "maf"))$single, gmmat_pval_likely_pheno)[1,]
match <- which(snp.meta(dat)$scaffold == top_snp$scaffold[1] & snp.meta(dat)$position == top_snp$position[1])
gmatch <- genotypes(dat)[match,]
gmatch <- data.frame(genotype = unlist(gmatch), phenotype = sample.meta(dat)$likely_pheno)
gmatch$genotype[gmatch$genotype == "NN"] <- NA
gmatch$genotype <- ifelse(gmatch$genotype == "GG", "resident", "migrant")
gtable <- table(gmatch)
gtable <- addmargins(gtable)
...
```{r Figure_1_print,echo=FALSE,warning=FALSE,fig.cap="Figure 1: Approximate ranges ancestral and introduced range with introduction dates for monarch butterflies in the Pacific, with sampling location noted in orange.",fig.width=8, fig.height=8, fig.align='center', fig.cap.style="Image Caption"}
