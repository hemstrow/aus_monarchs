...
Genetic diversity levels were extremely similar between our two years ($\pi =$ `r round(diversity$weighted_mean_pi[2], 4)` and `r round(diversity$weighted_mean_pi[3], 4)` and $H_{o}$ = `r round(diversity$weighted_mean_ho[2], 4)` and `r round(diversity$weighted_mean_ho[3], 4)` for 2016 and 2018, respectively). Each year had a slight heterozygote excess ($F_{IS}$ = `r round(diversity$weighted_mean_fis[2], 3)` and `r round(diversity$weighted_mean_fis[3],3)` for 2016 and 2018, respectively), although that excess was lower if all samples were considered jointly ($F_{IS}$ = `r round(diversity$weighted_mean_fis[1], 3)`). Pairwise $F_{ST}$ between the two years was small but marginally significant (`r round(fst$weighted_mean_fst[1], 3)`, $p =$ `r round(fst$weighted_mean_fst_p[1], 3)`). We found no significant genetic divergence between the two sample years on either the PCA or the NGSadmix results, with clustering instead driven by family, although we did find that three of our 2018 maternal families were substantially different from all other samples on our first two PC axes (Figure 2).
...
In contrast, take for example Chinook and steelhead salmon (*Oncorhynchus tshawytscha* and *mykiss*, respectively). Pre-mature migration, a migratory life history variant in both species where some portion of the population return to their natal streams earlier than others, seems to be controlled by an ancestrally derived (but independently evolved in each species) allelic variant at the GREB1L gene, which has been lost from the majority of populations due to hydrological changes such as dam installation [@Princee1603198; @Thompson177]. Salmon have *much* longer generation times and smaller effective population sizes (several years in both species), and so we should not expect pre-mature migration to as quickly re-evolve in salmon as it did migration in monarchs. In addition, since the pre-mature migration phenotypes have been selected against due to anthropogenic influences [@Thompson177], GREB1L is unlikely to be selectively neutral in most populations and is therefore not likely to be maintained over time as standing genetic variance.

Nonetheless, our findings are still optimistic for the future of migration in monarch butterflies or wherever selection on migratory alleles is merely relaxed and not becoming increasingly negative.
