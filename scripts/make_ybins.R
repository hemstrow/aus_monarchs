...
if(length(bad) > 0){sample.meta <- sample.meta[-bad,]}
...
write.table(ybin[,1], "data/relaxed_ybin_v4.txt", quote = F, col.names = F, row.names = F)
write.table(ybin[,2], "data/strict_ybin_v4.txt", quote = F, col.names = F, row.names = F)
write.table(ybin[,3], "data/likely_ybin_v4.txt", quote = F, col.names = F, row.names = F)
write.table(ybin[,4], "data/quant_ybin_v4.txt", quote = F, col.names = F, row.names = F)
